#cs
	...
	HTML_EvalJS()
	HTML_GUICtrlSetData()
	HTML_GUICtrlSetStyle()
	HTML_GUICtrlSetAttr()
	HTML_GUICtrlRemoveAttr()
...
#cs
	Tên hàm.........:    HTML_GUICtrlSetStyle()
	Mô tả:..........:    Style css cho element.
	Cú pháp.........:    HTML_GUICtrlSetAttr($id, $property, $value)
	Tham số.........:   -    $id     : ElementId
						-	 $property: Thuộc tính cần style (color, background-color, font-size, ... etc)
						-	 $value	 : Gía trị của thuộc tính đó (red, black, 16px, ... etc)
#ce
Func HTML_GUICtrlSetStyle($id, $property, $value)
   If($id == 'body') Then
	  $jsScript = "document.body.style." & $property & " = '"&$value&"';"
   Else
	  $jsScript = "document.getElementById('"&$id&"').style." & $property & " = '"&$value&"';"
   EndIf
	; Có thời gian mình sẽ sửa hàm này, cho phép style nhiều property một lúc
   HTML_EvalJS($jsScript)
EndFunc   ;==>HTML_GUICtrlSetStyle

#cs
	Tên hàm.........:    HTML_GUICtrlSetAttr()
	Mô tả:..........:    Sửa đổi attribute cho một ElementId.
	Cú pháp.........:    HTML_GUICtrlSetAttr($id, $attr, $value)
	Tham số.........:   -    $id     : ElementId
						-    $attr : Tên thuộc tính
						-    $attr : Gía trị thuộc tính
#ce
Func HTML_GUICtrlSetAttr($id, $attr, $value) ; set data
	$jsScript = "document.getElementById('"&$id&"').setAttribute('"&$attr&"', '"&$value&"');"
	HTML_EvalJS($jsScript)
EndFunc   ;==>HTML_GUICtrlSetAttr

#cs
	Tên hàm.........:    HTML_GUICtrlRemoveAttr()
	Mô tả:..........:    Xóa một attribute cho ElementId.
	Cú pháp.........:    HTML_GUICtrlSetAttr($id, $attr)
	Tham số.........:   -    $id     : ElementId
						-    $attr : Tên thuộc tính
#ce

Func HTML_GUICtrlRemoveAttr($id, $attr)
   $jsScript = "document.getElementById('"&$id&"').removeAttribute('"&$attr&"');"
   HTML_EvalJS($jsScript)
EndFunc ;==>HTML_GUICtrlRemoveAttr
#cs
